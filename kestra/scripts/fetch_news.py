...
CATEGORY_QUERIES = {
    ...
    "Politics": ["politics OR government OR election", "political news OR congress"],
    "Sports": ["sports OR football OR basketball", "NBA OR NFL OR soccer"],
    "Health & Science": ["health OR medicine OR healthcare", "science OR research"],
    "Entertainment": [
        "entertainment OR movies OR Hollywood",
        "celebrity OR entertainment news",
    ],
    "Gaming": ["gaming OR video games", "PlayStation OR Xbox"],
    "Environment": [
...
def main() -> None:
        ...
        try:
            ...
            if articles:
                ...
                for article in articles:
                    ...
                    category_counts[detected_cat] = (
                        category_counts.get(detected_cat, 0) + 1
                    )
    ...
    for category, count in sorted(
        final_categories.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    if processed_articles:
        ...
        for i, art in enumerate(processed_articles[:5]):
            log.info(
                f"   {i+1}. [{art.get('query_category', 'Unknown')}] {art.get('title', 'No title')}"
            )
