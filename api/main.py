...
async def record_interaction(interaction: dict):
    ...
    try:
        ...
        if existing_count == 0:
            ...
            print(
                f" EXISTS: Interaction already exists - User {interaction['user_id']} -> Article {interaction['article_id']}"
            )
            return {
                "status": "success",
...
def get_recommendations(user_id: int, limit: int = 5):
    ...
    if svd_model is None or user_mapping is None:
        print("Model not trained, returning popular articles as fallback")
    ...
    if user_id not in user_mapping.index:
        print(f"User {user_id} not in training data, returning popular articles")
